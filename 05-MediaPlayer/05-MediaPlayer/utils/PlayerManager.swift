...
protocol PlayUtilDelegate: class {
    
    func playerViewBack() // 返回
    func playFinished() // 播放完成
}

extension PlayUtilDelegate {
...
class PlayUtil: NSObject {    
    ...
    public weak var delegate: PlayUtilDelegate?
    ...
    private func initPlayerURL() {
        ...
        do {
            _ = try AVAudioSession.sharedInstance().setCategory(AVAudioSession.Category(rawValue: AVAudioSession.Category.playback.rawValue))
        } catch {
            print("")
        }
...
extension PlayUtil {// MARK: 外部调用方法
...
//                playerView.resizeTimeLabel()
    ...
    @objc fileprivate func statusBarOrientationChange(_ notification: Notification) {
        ...
        if orientation == UIDeviceOrientation.landscapeLeft {
            playerView.fullScreenWithDirection(.left, animate: false)
        } else if orientation == UIDeviceOrientation.landscapeRight {
            playerView.fullScreenWithDirection(.right, animate: false)
